...
export function createPlaybackUrl(asset: CatalogAsset, viewerId: string, gatewayOverride?: string, accessExpiresAt = Date.now() + 2 * 60 * 60 * 1000, leaseId = "") {
  ...
  const signature = hmac(`${tenant}.${asset.id}.${viewerId}.${leaseId}.${exp}.${accessExp}.${nonce}`, secret);
  ...
  if (leaseId) url.searchParams.set("lease", leaseId);
  url.searchParams.set("exp", String(exp));
  url.searchParams.set("accessExp", String(accessExp));
  url.searchParams.set("nonce", nonce);
  url.searchParams.set("sig", signature);
  return { url: url.toString(), expiresAt: exp * 1000, accessExpiresAt: accessExp * 1000, leaseId: leaseId || null, demo: false };
}
