...
export function createPlaybackUrl(asset: CatalogAsset, viewerId: string, gatewayOverride?: string) {
  ...
  const signature = hmac(`${tenant}.${asset.id}.${viewerId}.${exp}.${nonce}`, secret);
  ...
  return { url: url.toString(), expiresAt: exp * 1000, demo: false };
}
