...
import { audit, deleteAssets, listAssets, listProviders, setAssetsActive } from "@/lib/db";
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body.active) {
      if (assets.some((asset) => asset.isRestricted || !asset.isPlayable || asset.metadataReview === "needs_review")) throw new HttpError(409, "تتضمن القائمة محتوى يحتاج مراجعة أو غير قابل للتشغيل");
...
export async function DELETE(request: Request) {
  try {
    assertSameOrigin(request); requireAdminRequest(request);
    const body = await jsonBody<{ ids?: unknown; confirmation?: unknown }>(request, 80_000);
    if (!Array.isArray(body.ids) || !body.ids.length || body.ids.length > 500) throw new HttpError(400, "اختر من عنصر واحد إلى 500 عنصر للحذف");
    if (body.confirmation !== "delete-selected-content") throw new HttpError(400, "لم يتم تأكيد الحذف");
    const ids = [...new Set(body.ids.map((id) => cleanText(id, 80)))];
    const assets = (await listAssets(undefined, true)).filter((asset) => ids.includes(asset.id));
    if (assets.length !== ids.length) throw new HttpError(404, "بعض عناصر المحتوى غير موجودة");
    const deleted = await deleteAssets(ids);
    await audit("asset.bulk-delete", "asset", null, { deleted, requested: ids.length });
    revalidateTag("wiva-viewer-catalog", { expire: 0 });
    return Response.json({ ok: true, deleted, assets: await listAssets(undefined, true) }, { headers: { "cache-control": "no-store" } });
  } catch (error) { return errorResponse(error); }
}
