...
export default async function SeriesDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="container series-detail-page">
    ...
    {seasons.length ? seasons.map((season) => <section className="catalog-section" key={season}><div className="section-heading"><div><h2>الموسم {season.toLocaleString("ar")}</h2><p>{episodes.filter((episode) => episode.seasonNumber === season).length.toLocaleString("ar")} حلقة</p></div></div><div className="media-grid">{episodes.filter((episode) => episode.seasonNumber === season).map((episode) => <MediaCard key={episode.id} asset={episode} />)}</div></section>) : <div className="empty-state"><span><Layers3 /></span><h3>لا توجد حلقات منشورة</h3><p>يظهر المسلسل في الفهرس، لكن لم تُستورد حلقاته أو لم ينشرها المسؤول بعد.</p></div>}
  </div>;
}
