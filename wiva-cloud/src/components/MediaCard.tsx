...
export function MediaCard({ asset, priority = false }: { asset: CatalogAsset; priority?: boolean }) {
  ...
  return (
    <Link href={isSeries ? `/series/${asset.id}` : `/watch/${asset.id}`} className="media-card" aria-label={`${isSeries ? "فتح مسلسل" : "مشاهدة"} ${title}`} data-kind={asset.kind} data-priority={priority || undefined} prefetch={priority ? true : undefined}>
      <div className="media-art">
        ...
        <span className="play-float" aria-hidden="true">{asset.kind === "live" ? <Radio size={22} /> : <CirclePlay size={24} />}</span>
      </div>
      <div className="media-meta">
        <strong dir="auto">{title}</strong>
        <span dir="auto">{asset.category || (asset.kind === "live" ? "قنوات" : "مكتبة")}</span>
        <div>
