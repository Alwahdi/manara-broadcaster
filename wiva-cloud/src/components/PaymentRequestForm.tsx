...
export function PaymentRequestForm({ initial }: { initial: PaymentRequestSummary[] }) {
  const [requests, setRequests] = useState(initial); const [pending, setPending] = useState(false); const [message, setMessage] = useState("");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setRequests(payload.requests); form.reset(); setMessage("وصل طلبك. ستظهر النتيجة هنا بعد مراجعة الإدارة.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "تعذر إرسال الطلب"); }
    finally { setPending(false); }
  }
  return <section className="architecture-card payment-card" id="payment">
    <div className="ops-card-heading"><div><Send /><span><h2>طلب تجديد الاشتراك</h2><p>بعد إجراء الحوالة، أرسل المرجع للمراجعة. لا يوجد خصم تلقائي.</p></span></div></div>
    ...
    {hasPending ? <div className="watch-notice"><Clock3 /><span>لديك طلب قيد المراجعة. لا حاجة لإرساله مرة أخرى.</span></div> : <form className="stack-form payment-form" onSubmit={submit}>
      ...
      <button className="button primary" disabled={pending}>{pending ? <LoaderCircle className="spin" size={18} /> : <Send size={18} />}{pending ? "جارٍ الإرسال…" : "إرسال طلب المراجعة"}</button>
    </form>}
  </section>;
}
