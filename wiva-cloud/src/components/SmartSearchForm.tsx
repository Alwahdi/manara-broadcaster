...
export function SmartSearchForm({ query }: { query: string }) {
  ...
  return <><form className="search-form" action="/search" onSubmit={submit}><Search size={20} /><input name="q" value={value} onChange={(event) => setValue(event.target.value)} placeholder="اكتب اسم قناة أو فيلم أو مسلسل…" autoFocus aria-label="ابحث في المحتوى" />{value ? <button className="search-clear" type="button" onClick={() => setValue("")} aria-label="مسح البحث"><X /></button> : null}<button className="button primary">بحث</button></form>{!query && recent.length ? <div className="recent-searches"><span>عمليات بحث سابقة</span><div>{recent.map((item) => <Link key={item} href={`/search?q=${encodeURIComponent(item)}`}><Search />{item}</Link>)}</div></div> : null}</>;
}
