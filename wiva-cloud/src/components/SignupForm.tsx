...
export function SignupForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "تعذر إنشاء الحساب الآن"); requestAnimationFrame(() => errorRef.current?.focus()); }
    finally { setPending(false); }
  }
  return <form className="auth-form" onSubmit={submit} aria-busy={pending}>
    <label htmlFor="signup-name">الاسم<input id="signup-name" name="name" autoComplete="name" maxLength={120} enterKeyHint="next" placeholder="اكتب اسمك…" required /></label>
    <label htmlFor="signup-email">البريد الإلكتروني<input id="signup-email" name="email" type="email" dir="ltr" autoComplete="email" inputMode="email" enterKeyHint="next" spellCheck={false} placeholder="name@example.com…" required aria-invalid={Boolean(error)} /></label>
