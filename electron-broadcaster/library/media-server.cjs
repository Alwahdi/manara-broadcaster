...
function adminLoginPage(error = '', basePath = '/admin') {
  return `<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#080b12"><title>دخول إدارة WIVA</title><style>
:root{color-scheme:dark;--bg:#080b12;--panel:rgba(17,23,34,.78);--line:rgba(255,255,255,.1);--text:#f6f8fb;--muted:#a7b0bd;--brand:#d8c69e;--brand-soft:#f3e7ca;--teal:#7edbd3}
*{box-sizing:border-box}html,body{margin:0;min-height:100%}body{min-height:100vh;min-height:100dvh;display:grid;place-items:center;padding:max(20px,env(safe-area-inset-top)) max(16px,env(safe-area-inset-right)) max(20px,env(safe-area-inset-bottom)) max(16px,env(safe-area-inset-left));overflow-x:hidden;color:var(--text);font-family:"Cairo",system-ui,-apple-system,Segoe UI,Tahoma,sans-serif;background:radial-gradient(64rem 48rem at 90% -12%,rgba(126,219,211,.11),transparent 60%),radial-gradient(48rem 40rem at -10% 18%,rgba(174,185,236,.08),transparent 64%),linear-gradient(180deg,#080b12,#070a10)}
body:before{content:"";position:fixed;inset:0;pointer-events:none;opacity:.13;background-image:linear-gradient(rgba(255,255,255,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.025) 1px,transparent 1px);background-size:56px 56px;mask-image:linear-gradient(to bottom,#000,transparent 76%)}
.login{position:relative;width:min(420px,100%)}.mark{display:flex;align-items:center;gap:12px;margin:0 4px 14px}.mark img{width:44px;height:44px;border-radius:13px;object-fit:contain;box-shadow:0 0 0 1px rgba(255,255,255,.13),0 16px 34px -24px rgba(216,198,158,.8)}.brand b{display:block;font-size:17px}.brand span{display:block;margin-top:1px;color:var(--muted);font-size:12px}
.card{padding:clamp(20px,6vw,28px);border:1px solid var(--line);border-radius:22px;background:linear-gradient(145deg,rgba(22,30,43,.8),rgba(12,17,26,.76));box-shadow:inset 0 1px rgba(255,255,255,.055),0 32px 90px -44px #000;backdrop-filter:blur(24px) saturate(135%);-webkit-backdrop-filter:blur(24px) saturate(135%)}h1{margin:0 0 7px;font-size:clamp(24px,7vw,30px);letter-spacing:-.02em}.lead{margin:0 0 20px;color:#c3cad3;font-size:13px;line-height:1.8}label{display:block;margin-top:13px;color:#e5e9ef;font-size:12px;font-weight:900}input{width:100%;min-height:48px;margin:7px 0 2px;padding:12px 14px;border:1px solid rgba(255,255,255,.14);border-radius:13px;outline:none;color:#fff;background:rgba(7,11,18,.72);box-shadow:inset 0 1px rgba(255,255,255,.03);font:inherit;transition:border-color .16s ease,box-shadow .16s ease}input:focus{border-color:var(--teal);box-shadow:0 0 0 3px rgba(126,219,211,.2)}button{width:100%;min-height:48px;margin-top:18px;padding:12px;border:0;border-radius:13px;cursor:pointer;color:#0b1018;background:linear-gradient(135deg,var(--brand-soft),var(--brand) 60%,#ad9262);box-shadow:0 18px 42px -28px rgba(216,198,158,.82),inset 0 1px rgba(255,255,255,.5);font:inherit;font-weight:900}.err{padding:10px 12px;border:1px solid rgba(248,113,113,.34);border-radius:12px;color:#fecaca;background:rgba(127,29,29,.24);font-size:13px;line-height:1.6}.note{margin:14px 0 0;color:var(--muted);font-size:11px;line-height:1.7;text-align:center}@media(max-width:360px){body{padding-inline:12px}.card{padding:18px 16px;border-radius:18px}.lead{font-size:12px}}
@supports not ((backdrop-filter:blur(1px)) or (-webkit-backdrop-filter:blur(1px))){.card{background:#111722}}
</style></head><body><main class="login"><div class="mark"><img src="/wiva-logo.png" alt="WIVA"><div class="brand"><b>WIVA</b><span>إدارة الشبكة المحلية</span></div></div><form class="card" method="post" action="${escapeHtml(basePath)}/login"><h1>تسجيل الدخول</h1><p class="lead">ادخل ببيانات الإدارة لإدارة القنوات، IPTV، المكتبة، المشاهدين، والتقارير.</p>${error ? `<p class="err">${escapeHtml(error)}</p>` : ''}<label>اسم المستخدم<input name="username" autocomplete="username" placeholder="admin" required autofocus></label><label>كلمة المرور<input name="password" type="password" autocomplete="current-password" placeholder="كلمة المرور" required></label><button>دخول إلى اللوحة</button><p class="note">الجلسة محفوظة على هذا الجهاز لمدة أسبوع.</p></form></main></body></html>`;
}
