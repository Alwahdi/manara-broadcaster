...
.mobile-filter-toggle{display:none;border:1px solid var(--line2);background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;border-radius:var(--radius);padding:10px 13px;min-height:46px;font-weight:950;cursor:pointer;white-space:nowrap}
...
.section-card{position:relative;overflow:hidden;border:1px solid var(--line);background:linear-gradient(160deg,rgba(17,24,39,.9),rgba(15,23,42,.58));border-radius:var(--radius2);padding:16px;text-align:start;color:#fff;min-height:190px;transition:transform .18s,border-color .18s,box-shadow .18s}.section-card.has-cover{background-image:linear-gradient(180deg,rgba(7,9,15,.26),rgba(7,9,15,.9)),var(--folder-cover);background-size:cover;background-position:center}.section-card::before{content:"";position:absolute;inset:auto -20% -42% -20%;height:120px;background:radial-gradient(circle,rgba(20,184,166,.2),transparent 62%)}.section-card:hover{transform:translateY(-4px);border-color:rgba(20,184,166,.5);box-shadow:0 18px 42px rgba(0,0,0,.28)}.section-main{width:100%;height:100%;border:0;background:transparent;color:#fff;text-align:start;padding:0;font:inherit;cursor:pointer;position:relative;z-index:1;display:flex;flex-direction:column;justify-content:flex-end}.section-main strong{display:block;font-size:17px}.section-main span{display:block;color:#dbeafe;font-size:12px;margin-top:5px}.folder-row{display:flex;gap:7px;flex-wrap:wrap;margin-top:13px;position:relative;z-index:1}.folder-row button{border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.34);color:#dbeafe;border-radius:999px;padding:6px 9px;font:inherit;font-size:11px;font-weight:900;cursor:pointer;backdrop-filter:blur(8px)}
.folder-toolbar{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:12px}.folder-toolbar-actions{display:flex;gap:8px;flex-wrap:wrap}.breadcrumb{display:flex;gap:7px;flex-wrap:wrap}.breadcrumb button{border:1px solid var(--line);background:rgba(255,255,255,.06);color:#e5edff;border-radius:999px;padding:7px 10px;font:inherit;font-size:12px;font-weight:900;cursor:pointer}.folder-tree-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:14px}.folder-tree-grid.list{grid-template-columns:1fr}.folder-card{border:1px solid var(--line);background:linear-gradient(150deg,rgba(17,24,39,.94),rgba(15,23,42,.66));border-radius:var(--radius2);min-height:248px;padding:16px;text-align:start;color:#fff;cursor:pointer;transition:transform .18s,border-color .18s,box-shadow .18s;position:relative;overflow:hidden;box-shadow:0 12px 34px rgba(0,0,0,.18);display:flex;flex-direction:column;justify-content:space-between}.folder-card::after{content:"";position:absolute;inset:auto -35% -45% -35%;height:130px;background:radial-gradient(circle,rgba(248,197,28,.18),transparent 64%);pointer-events:none}.folder-card.has-cover{background-size:cover;background-position:center}.folder-card:hover{transform:translateY(-3px);border-color:rgba(248,197,28,.5);box-shadow:0 18px 46px rgba(0,0,0,.3)}.folder-card-head{position:relative;z-index:1;display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:12px}.folder-symbol{width:62px;height:62px;display:grid!important;place-items:center;border-radius:20px;background:rgba(248,197,28,.13);border:1px solid rgba(248,197,28,.24);color:var(--accent)!important;box-shadow:0 12px 34px rgba(0,0,0,.18)}.folder-symbol svg{width:31px;height:31px;stroke:currentColor;fill:none;stroke-width:1.9;stroke-linecap:round;stroke-linejoin:round}.folder-chip{display:inline-flex!important;align-items:center;gap:5px;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.34);color:#dbeafe!important;border-radius:999px;padding:5px 9px;font-size:10px!important;font-weight:950;backdrop-filter:blur(8px)}.folder-copy{position:relative;z-index:1}.folder-card b{display:block;font-size:16px;margin-bottom:7px}.folder-card span{display:block;color:var(--muted);font-size:12px;line-height:1.7}.folder-card small{display:inline-flex;margin-top:8px;border:1px solid rgba(248,113,113,.25);background:rgba(127,29,29,.28);color:#fecaca;border-radius:999px;padding:4px 8px;font-size:10px;font-weight:950}.folder-card.file{cursor:default}.folder-card.file .folder-symbol{background:rgba(59,130,246,.12);border-color:rgba(147,197,253,.22);color:#bfdbfe!important}.folder-card.file a{display:inline-flex;margin-top:10px;color:#fde68a;text-decoration:none;font-weight:950}.folder-tree-grid.list .folder-card{min-height:82px;display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center}.folder-tree-grid.list .folder-card .folder-card-head{margin:0}.folder-tree-grid.list .folder-card a{margin-top:0}
...
<main class="library-shell">
  <div class="tools">
    ...
    <button class="mobile-filter-toggle" id="filterToggle" type="button" aria-expanded="false">${escapeHtml(text.filters)}</button>
  ...
  <section class="section" id="libraryResultsSection"><div class="section-head"><div><h2>${escapeHtml(text.library)}</h2><small id="countLabel"></small></div><i class="section-line"></i></div><div class="grid" id="grid"></div><div class="empty" id="empty">${escapeHtml(text.empty)}</div></section>
</main>
<script>
...
function openFolderPath(parts){
  folderPath = Array.isArray(parts) ? parts.filter(Boolean) : [];
  renderFolderTree();
  document.getElementById('folderTreeSection').scrollIntoView({ behavior:'smooth', block:'start' });
}
let heroIndex = 0;
let heroTimer = 0;
...
function render(){
  ...
  const showFlatResults = hasActiveFlatFilter();
  document.getElementById('libraryResultsSection').hidden = !showFlatResults;
...
filterToggle.addEventListener('click', () => {
  ...
  filterToggle.textContent = open ? (text.closeFilters || 'Hide filters') : (text.filters || 'Filters');
});
...
function renderSectionRail(){
  const root = folderItemsAt([]);
  const rows = root.dirs.map((dir) => ({
    name: dir.name,
    count: dir.count,
    cover: dir.cover || assetUrl('source'),
    parts: [dir.name],
    label: dir.sourceLabel || dir.name,
  }));
  const rail = document.getElementById('sectionRail');
  document.getElementById('sectionBrowser').hidden = rows.length === 0;
  rail.innerHTML = rows.map(row => '<div class="section-card '+(row.cover?'has-cover':'')+'" '+(row.cover?'style="--folder-cover:url(\\''+esc(row.cover)+'\\')"':'')+'><button class="section-main" data-open-path="'+esc(JSON.stringify(row.parts))+'"><strong>'+esc(row.name)+'</strong><span>'+esc([row.label && row.label !== row.name ? row.label : '', row.count + ' ' + (text.items || '')].filter(Boolean).join(' · '))+'</span></button></div>').join('');
  rail.querySelectorAll('[data-open-path]').forEach((btn) => btn.onclick = () => {
    try { openFolderPath(JSON.parse(btn.dataset.openPath || '[]')); } catch { openFolderPath([]); }
  });
}
renderSectionRail();
document.querySelectorAll('[data-quick-view]').forEach(btn => btn.onclick = () => {
  ...
  render();
  document.getElementById(hasActiveFlatFilter() ? 'libraryResultsSection' : 'folderTreeSection').scrollIntoView({ behavior:'smooth', block:'start' });
});
