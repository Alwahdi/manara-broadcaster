...
function agentState() {
  ...
  return {
    ...
    networkLogoDataUrl: settings.networkLogoDataUrl,
  ...
  return {
    ...
    applySetup: async (patch = {}) => {
      ...
      const next = {
        ...
        networkName: String(clean.networkName ?? settings.networkName ?? '').trim(),
        networkNumber: String(clean.networkNumber ?? settings.networkNumber ?? '').trim(),
        networkLocation: String(clean.networkLocation ?? settings.networkLocation ?? '').trim(),
        networkCountry: String(clean.networkCountry || clean.country || settings.networkCountry || '').trim(),
        networkRegion: String(clean.networkRegion ?? settings.networkRegion ?? '').trim(),
