...
- Legacy `adminPage()` and the legacy setup wizard are an emergency developer fallback only. The explicit `/admin/legacy` and `/setup/legacy` routes return `404` in normal production use and are re-enabled solely via the `WIVA_ALLOW_LEGACY_UI` environment flag; the smoke test asserts they are unreachable by default. The server-rendered pages still back the normal routes when `webui/dist` has not been built yet.- Public media library with viewer accounts, favorites, watch later, history, and messages.
